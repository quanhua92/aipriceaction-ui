...
test.describe('Ticker Performance Table', () => {
  test('should display sortable table in Portfolio page', async ({ page }) => {
    ...
    await page.goto('/portfolio?tickers=["VCB","BID","CTG"]&range=3M&lang=en');
  ...
  test('should display sortable table in Compare page', async ({ page }) => {
    ...
    await page.goto('/compare?tickers=["HPG","HSG"]&range=1M&lang=en');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(3000); // Wait for data to load
  ...
  test('should handle sorting functionality', async ({ page }) => {
    ...
    await page.goto('/portfolio?tickers=["VCB","BID","CTG","ACB"]&range=3M&lang=en');
    await page.waitForLoadState('networkidle');
    ...
    // Click to sort by ticker and verify sort icon appears
    await tickerSortButton.click();
    await page.waitForTimeout(500); // Wait for sort to apply
    
    // Verify ticker sort has been applied by checking first ticker in alphabetical order
    const firstTickerRow = page.locator('tbody tr').first();
    await expect(firstTickerRow).toContainText('ACB'); // ACB should be first alphabetically
    ...
    // Verify sorting controls are functional (table should still be visible after sorting)
    await expect(page.locator('text=Portfolio Stocks Performance')).toBeVisible();
  });
  ...
  test('should show performance data with colors', async ({ page }) => {
    ...
    await page.goto('/portfolio?tickers=["VCB","VNINDEX"]&range=3M&lang=en');
    await page.waitForLoadState('networkidle');
